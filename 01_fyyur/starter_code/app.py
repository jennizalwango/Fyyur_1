...
def artists():
    ...
    artists = db.session.query(Artist.filter_by(Artist.id, Artist.name)).all()
...
def search_artists():
    ...
    search_term = request.form.get('search_term', '')
    venues = db.session.query(Venue).with_entites(Venue.id, Venue.name.filter(Venue.name.contains(search_term))).all()
    data = []
    for v in venues:
...
def shows():
    ...
    for s in shows:
        ...
        show['artist_image'] = s.artist.image_link
...
def create_show_submission():
    ...
    try:
        ...
        venune_id = request.form.get('venue_id', '')
